...
@Service
@Slf4j
public class MailRoomOrderProcessServiceImpl implements MailRoomOrderProcessService {
    ...
    private static long TIMEOUT = 70000;
    @Override
    @Transactional
    public HashMap<String, Object> process() throws Exception {

        log.debug("Running MailRoomOrderProcessServiceImpl->process");
...
//        }
        ...
        if (CollectionUtils.isNotEmpty(allOrders)) {
            List<List<FulfillmentOrder>> orderChunks = ListUtils.partition(allOrders, Integer.valueOf(195));
                ...
                if (IfPdfFound) {
                    try {
                        ...
                        for (FulfillmentOrder updateOrder : ordersToUpdate) {
                            ...
                            Long order_id = Long.valueOf(updateOrder.getId());
                            ShippingLabels shippingLabelsPackage = new ShippingLabels();
                            shippingLabelsPackage.setOrderId(order_id);
                            try {
                                ...
                                adminSharedService.mapLabelResponseToShippinLabel(order_id,
                                        labelPackageResponse, shippingLabelsPackage);
                                shippingLabelsPackages.add(shippingLabelsPackage);
                            } catch (HttpStatusCodeException exception) {
                                if (exception.getStatusCode() == HttpStatus.TOO_MANY_REQUESTS) {
                                    log.error("ERROR: ShipEngine Error Too Many Request : 1922 " + order_id, exception);
                                    Thread.sleep(TIMEOUT);
                                    try {
                                        LabelPackageEnvelopeResponse labelPackageResponse = adminSharedService
                                                .postLabelPackageRequest(updateOrder);
                                        ...
                                        erroredShippingPackageOrderNumbers.add(order_id);
                                    ...
                                    log.error("ERROR: Shipping labelPackage error: mailRoomProcess(): 1943 "
                                            + order_id + " " + exception.getResponseBodyAsString());
                                    System.out.println(adminSharedService.createLabelPackageRequestData(updateOrder));
                                }
                            } catch (Exception e) {
                                log.error("ERROR: While create the Batch: 1961 : OrderNum" + order_id + " ", e);
                            }

                            ShippingLabels shippingLabelsEnvelop = new ShippingLabels();
                            shippingLabelsEnvelop.setOrderId(order_id);
                            try {
                                ...
                                if (exception.getStatusCode() == HttpStatus.TOO_MANY_REQUESTS) {
                                    ...
                                    try {
                                        ...
                                        erroredShippingEnvelopOrderNumbers.add(order_id);
                                    }
                                } else {
                                    shippingLabelsEnvelop.setErrorMessage(exception.getResponseBodyAsString());
                                    shippingLabelsRepo.save(shippingLabelsEnvelop);
                        ...
                        String uploadedShippingLabelPath = uploadFileToAwsService.upload(
