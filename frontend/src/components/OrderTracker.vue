<template>
  <div class="casetracker-wrap pt-5 pb-5">
    <div class="faq-title">Order Tracker</div>
    <div class="faq-desc mb-3">
      Watch our tax specialists complete your order in real time.
    </div>
        ...
        <div class="status-show">
          ...
          <div class="status-text" v-else>
            <a
              ...
              "
              target="_blank"
              style="text-decoration: none; color: #2a96c4"
              >Work Complete</a
            >
          </div>
          ...
          <div class="orderstatus-text mt-3" v-if="!hasError">
            <div v-if="orderStatus === 2">
              ...
              <div v-if="!isEin">
                Your order {{ orderNumber }} went through correctly and we're
              ...
              <div v-if="isEin">
                Your order {{ orderNumber }} went through correctly and we're
                preparing EIN request to IRS now. It will take us about 1 business day
                to email results to you, so you may close out of this
                window now. Further updates will be sent to your email address at
                <b
                  ><span class="blue-text">{{ customerEmail }}</span></b
                >.
            ...
            <div v-if="orderStatus === 3">
              ...
              <div v-if="!isEin">
                ...
                first through the <b>Post Office</b> (the {{ partyName }} requires
                your physical signature). We'll send you an email soon with the
              ...
              <div v-if="isEin">
                We are preparing your EIN request to IRS now and will have it completed
                within the next business day. We'll send you an email soon with the
                <b>Employer Identification Number</b> from the <b>IRS</b> at
                <b
                  ><span class="blue-text">{{ customerEmail }}</span></b
                >
                once it is complete.
            ...
            <div v-if="orderStatus === 4">
              ...
              <div v-if="!isEin">
                ...
                you first through the <b>Post Office</b> (the
                {{ partyName }} requires your physical signature on the
                application). We'll send you an email at
                <b
                  ><span class="blue-text">{{ customerEmail }}</span></b
                >
                within the next business day when it is complete and in the mail,
                along with the <b>USPS tracking number</b> from the
                <b>Post Office</b>. <br /><br />NOTE: We can only mail orders and
                create <b>USPS tracking numbers</b> Monday-Friday. No weekends or
                federal holidays.
              </div>
              <div v-if="isEin">
                We have your request prepared, we're just doing the final checking.
                We'll send you an email at
                <b
                  ><span class="blue-text">{{ customerEmail }}</span></b
                >
                within the next business day when it is complete,
                along with the <b>Employer Identification Number</b>.
                <br /><br />NOTE: We can only do EIN requests Monday-Friday.
            ...
            <div v-if="orderStatus === 5">
              ...
              <div v-if="isEin">
                We have your order complete, and we have sent email with results.
                The <b>Employer Identification Number</b> is: {{trackingNumber}}.
              </div>
            </div>
...
export default {
  ...
  data() {
    return {
      ...
      orderStatusText: "",
      orderStatusDisplayText: "",
      orderNumber: this.defaultOrderNumber || "",
  ...
  methods: {
    ...
    trackOrder: function () {
      ...
      if (this.orderNumber.trim() == "" || this.phoneNumber.trim() == "") {
        this.hasError = true;
        this.errorMsg = "Please enter order number and phone number.";
        return;
      }
      this.orderStatus = 0;
      AXIOS.get(
        `/casetracker/order/${this.orderNumber.trim()}/${this.phoneNumber.trim()}`
      )
        .then((response) => {
          const {
            status,
          ...
          console.log(this.isIllinois, this.orderStatus);
          this.piWidth = "calc(" + (this.orderStatus * 20 + "%") + " - 56px";

          if (this.orderStatus <= 0) {
            if (this.orderNumber.trim() == "" || this.phoneNumber.trim() == "")
              this.orderStatusDisplayText = "";
            else this.orderStatusDisplayText = "Order Confirmed";
