  ...
  <page>
    <div class="container">
      <div class="row">
        <div class="col-12">
          <div v-if="success">
            <h1 class="mt-4 py-3 text-center font-weight-bold text-uppercase">
              Thank you
            </h1>
            <p>
              Thank you for your order. Your payment of ${{ amountDollars }} was
              successful and we'll begin working on your case now. Your order
              number is <b>{{ confirmation.orderId }}</b
              >. Please check your other order details below:
            </p>
            <div class="upsell-product" v-if="upsellProductData.isShow">
              ...
              <div class="row">
                ...
                <div class="col-7 pb-2">
                  ...
                  <span v-else>
                    Do you also owe a balance to the
                    <b>{{ upsellProductData.messageDepartment }}</b
                    >? It so, we can also help you set up a payment plan with
                    {{ upsellProductData.messageState }}.</span
                  >
                </div>
                <div class="col-3">
                  <button
                    ...
                    v-on:click="
                      updateUpsellProduct(
                        upsellProductData.productName,
                        upsellProductData.route
                      )
                    "
                  >
                ...
                <div class="col-12 col-sm-6">
                  <div class="info-block">
                    ...
                    <div class="info-value">
                      ...
                      <div>
                        {{ confirmation.shippingState }}
                        {{ confirmation.shippingZip }}
                      </div>
                    </div>
                  </div>
                  <div class="info-block">
                    <div class="info-value">
                      <div>
                        Largest Penalty Eligible for Waiver:
                        {{ confirmation.penaltyType }}
                      </div>
                      <div>
                        Largest Penalty Tax Year: {{ confirmation.penaltyYear }}
                      </div>
                      <!--
                      <div>
                        Total Amount Owed: ${{ confirmation.highestPenalty }}
                      </div>
                      -->
                    </div>
                  </div>
            ...
            <div class="explanation">
              <p>
                We have also sent you an email confirmation with the above
                information. If any of this information is incorrect, please use
                our contact form now to let us know so we can update it right
                away.
              </p>
              <p>
                We are preparing your documents now and will have them complete
                and in the mail to you within the next business day (Monday -
                Friday). We will mail this directly to you first through the
                <b>Post Office</b> (your physical signature is required) and
                email you with the <b>USPS tracking number</b> once it's on the
                way.
              </p>
              <p>
                You can get updates by entering your order number and phone
                number into the order tracker on our homepage.
              </p>
            </div>
          </div>
          <div v-if="!success">
...
export default {
  ...
  mounted() {
    AXIOS.get(
      `/ecommerce/confirmation/penaltyWaiver/${this.$route.params.correlationId}`
    )
      .then((response) => {
        ...
        imagetag.src =
          "https://www.shareasale.com/sale.cfm?tracking=" +
          this.confirmation.orderId +
          "&amount=" +
          this.amountDollars +
          "&merchantID=76439&transtype=sale&sscidmode=6&sscid=" +
          this.shareasaleSSCID;
        imagetag.width = "1";
        imagetag.height = "1";
        ...
        document
          .getElementById("app")
          .insertAdjacentElement("beforebegin", imagetag);
        document
          .getElementById("_SHRSL_img_1")
        ...
        AXIOS.get(
          `/ecommerce/orders/listInProcessAndComplete/${this.confirmation.orderId}`
        )
          .then((orderResponse) => {
  ...
  methods: {
    ...
    updateUpsell() {
      AXIOS.put(
        `/ecommerce/order/id/${this.confirmation.orderId}/update`,
        pick(this.confirmation, [
          "upsellProduct",
          "upsellClicked",
          "upsellShown",
        ])
    ...
    updateUpsellShown() {
      if (
        this.confirmation.upsellShown == 0 ||
        !this.confirmation.upsellShown
      ) {
    ...
    checkIsIRSPayment() {
      return (
        ...
        this.confirmation.orderId &&
        !(
          this.confirmation.isCalifornia ||
          this.confirmation.isNewJersey ||
          this.confirmation.isGeorgia ||
          this.confirmation.isIllinois
        )
      );
    },
    ...
    checkUserIsMarriedAndSeparately() {
      return this.confirmation &&
        this.confirmation.married &&
        this.confirmation.filingJointly == this.marriedFilingOptions.separately
        ? true
        : false;
    },
    setUpsell(orders) {
